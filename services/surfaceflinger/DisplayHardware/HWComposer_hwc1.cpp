...
#ifdef OMAP_ENHANCEMENT_HWC_EXTENDED_API
int HWComposer::hook_extension_cb(struct hwc_procs* procs __unused, int operation __unused,
...
int HWComposer::extendedApiDisplayInfo(hwc_display_info* param) {
    ...
    if (dpy > 31 || !mAllocatedDisplayIDs.hasBit(dpy))
        return BAD_INDEX;
    param->width = getWidth(dpy);
    param->height = getHeight(dpy);
    return NO_ERROR;
}
